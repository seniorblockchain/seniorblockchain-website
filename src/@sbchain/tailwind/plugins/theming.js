...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--sbchain-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--sbchain-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--sbchain-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ SBCHAIN TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--sbchain-${e(key)}`, value], [`--sbchain-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`sbchain.customProps.background.${colorScheme}`);
            const foreground = theme(`sbchain.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--sbchain-${e(key)}`, value], [`--sbchain-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--sbchain-${e(key)}`, value], [`--sbchain-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
