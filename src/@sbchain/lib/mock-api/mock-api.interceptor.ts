...
export class SBChainMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(SBCHAIN_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _sbchainMockApiService: SBChainMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._sbchainMockApiService.findHandler(request.method.toUpperCase(), request.url);
